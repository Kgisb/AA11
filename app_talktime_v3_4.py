...
APP_TITLE = "📞 TalkTime App — v3.4 (Agent, Country, B2C Team)"
TZ = "Asia/Kolkata"
...
# Targets provided by you (B2C team). We'll fuzzy-match.
B2C_TARGETS_RAW = [
    "Kamaldeep singh",
    "Ria Arora",
    "ayushman jetlearn",
...
def looks_like_b2c(caller_name, ratio_cut=0.85):
    n = norm_name(caller_name)
    if not n:
        return False
    for t in B2C_TARGETS:
        if not t:
            continue
        # Direct containments either way
        if t in n or n in t:
            return True
...
with st.sidebar:
    ...
    team_mode = st.radio("Choose agents to analyze", ["All agents", "B2C team only"], help="B2C team includes: " + ", ".join(B2C_TARGETS_RAW))
...
# Date-only for counting & filtering
...
# Optional datetime for hour visuals
...
# Sidebar filters (choices) — default to ALL selected
with st.sidebar:
    # Agent (Caller)
    if "Caller" in df.columns:
        agents = sorted(df["Caller"].dropna().astype(str).unique().tolist())
        # Don't show massive agent picker if B2C-only is selected; we still show for transparency
...
# Apply filters — preserve rows with NaN when include_missing=True
...
# First, Agent Set radio overrides agent selection if B2C team chosen
if team_mode == "B2C team only" and "Caller" in df_f.columns:
    b2c_mask = df_f["Caller"].apply(looks_like_b2c)
    if include_missing:
        b2c_mask = b2c_mask | df_f["Caller"].isna()
    df_f = df_f[b2c_mask].copy()
else:
...
st.caption(f"Agent Set: **{team_mode}** | Mode: **{mode}** | Threshold: **{threshold}s** | Range: **{preset}** (IST date-based)")
...
st.caption("New: **Agent Set** radio to choose 'All agents' or 'B2C team only' with fuzzy name matching. Rows preserved even with missing fields; date-based counting.")
