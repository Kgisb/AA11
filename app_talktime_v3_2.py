...
APP_TITLE = "📞 TalkTime App — v3.2 (Agent & Country Analytics)"
TZ = "Asia/Kolkata"
...
def parse_date_best(series):
    """Parse Date robustly: test dayfirst True vs False, keep the one with more valid rows."""
...
def combine_date_time(date_col, time_col):
    """Combine Date + Time (if possible) into tz-aware datetime for hour visuals only."""
...
def filter_by_date(df, start_date, end_date):
    """Inclusive date filter using parsed _date_only.
       Keep rows where _date_only falls in the window; other columns may be NaN and are preserved.
    """
...
expected = ["Date", "Time", "Caller", "Country Name", "Call Status", "Call Duration"]
...
def _apply_filter(frame, col, selections):
    if selections is None or len(selections) == 0 or col not in frame.columns:
        return frame
    if include_missing:
...
st.caption(f"Mode: **{mode}** | Threshold: **{threshold}s** | Range: **{preset}** (IST date-based)")
...
with tab4:
    ...
    # For 24h visuals, use rows that have _hour; but still respect *other* filters via df_f
    ...
    if not df_time.empty and df_time["_hour"].notna().any():
        # Attempts by hour (based on filtered dataset before duration threshold, as per 'attempts')
        attempts = (df_time
...
st.caption("Rows are preserved even if Agent/Country/Type/Status are blank (toggle in sidebar). Only Date is required for date filtering; Time is used for hour visuals when available.")
