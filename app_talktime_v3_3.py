...
APP_TITLE = "📞 TalkTime App — v3.3 (Agent & Country Analytics)"
TZ = "Asia/Kolkata"
...
def clean_str_col(series):
    # Preserve NaN; strip spaces on non-null
    if series is None:
        return series
    s = series.copy()
    try:
...
def filter_multiselect(label, key, options):
    ...
    with c1:
        sel = st.multiselect(label, options, default=st.session_state[key], key=f"ms_{key}")
    with c2:
        if st.button("All", key=f"all_{key}"):
            st.session_state[f"ms_{key}"] = options.copy()
            sel = options.copy()
    with c3:
...
# Clean up whitespace in key columns so " Ali " and "Ali" match
...
with st.sidebar:
    ...
    if "Caller" in df.columns:
        ...
        sel_agents = filter_multiselect("Agent(s)", "agents", agents)
    else:
        sel_agents = None
    ...
    if "Country Name" in df.columns:
        ...
        sel_countries = filter_multiselect("Country(ies)", "countries", countries)
    else:
        sel_countries = None
    ...
    if "Call Type" in df.columns:
        ...
        sel_types = filter_multiselect("Call Type(s) (optional)", "types", call_types)
    else:
        sel_types = None
    ...
    if "Call Status" in df.columns:
        ...
        sel_status = filter_multiselect("Call Status", "status", statuses)
    else:
        sel_status = None
...
def _apply_filter(frame, col, selections):
    if selections is None or col not in frame.columns:
        return frame
    if len(selections) == 0:
        # nothing selected -> no filtering (treat as "all")
        return frame
    if include_missing:
...
st.caption("Fixes: Filters default to ALL selections; 'All/Clear' buttons added; key columns trimmed to avoid hidden mismatches. Include-missing toggle keeps rows with blanks.")
