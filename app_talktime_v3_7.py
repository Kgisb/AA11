...
APP_TITLE = "📞 TalkTime App — v3.7 (Agent Set radio + Date/Time window)"
TZ = "Asia/Kolkata"
...
def combine_date_time(date_col, time_col):
    ...
    # Try parsing time flexibly; if not parseable, keep NaT
    t_try = pd.to_datetime(time_col, errors="coerce")
    t = t_try.dt.time if hasattr(t_try, "dt") else None
    # Build string safely; if t is None, result will be "date None" -> coerce to NaT
...
with st.sidebar:
    ...
    if preset in ["Today", "Yesterday"]:
        ...
        custom_dates = st.date_input("Custom dates (start & end, inclusive)", value=None)
...
df["_date_only_full"] = parse_date_best(df["Date"]) if "Date" in df.columns else pd.NaT  # timezone-naive
df["_date_only"] = pd.to_datetime(df["_date_only_full"]).dt.date

# Optional time for hour visuals & datetime filter
...
if preset == "Today":
    ...
    if not custom_dates or (isinstance(custom_dates, (list, tuple)) and len(custom_dates) != 2):
        st.warning("Select a start and end date for Custom.")
        start_date, end_date = (now.date(), now.date())
    else:
        # streamlit returns datetime.date or list
        if isinstance(custom_dates, (list, tuple)):
            start_date, end_date = custom_dates[0], custom_dates[1]
        else:
            start_date, end_date = custom_dates, custom_dates

# Build tz-aware datetimes for the window
start_dt = pd.Timestamp.combine(start_date, t_start).tz_localize(TZ)
end_dt   = pd.Timestamp.combine(end_date,   t_end).tz_localize(TZ)

# Apply datetime-aware filter
df_f = df.copy()
# Primary mask using _dt_local when available
...
# Fallback mask using date-only for rows with missing _dt_local (if allowed)
...
# --------------------
# Team filter (radio with three options)
# --------------------
B2C_NORM = [norm_name(x) for x in B2C_TARGETS_RAW]
MT_NORM  = [norm_name(x) for x in MT_TARGETS_RAW]
...
if team_mode == "B2C team only":
    mask_team = mask_for_targets(df_f, "Caller", B2C_NORM)
    if include_missing:
        mask_team = mask_team | df_f["Caller"].isna()
    df_f = df_f[mask_team].copy()
elif team_mode == "MT Team only":
    mask_team = mask_for_targets(df_f, "Caller", MT_NORM)
    if include_missing:
        mask_team = mask_team | df_f["Caller"].isna()
    df_f = df_f[mask_team].copy()
# else: All agents -> no team mask
...
with tab4:
    ...
    # Use pre-filtered df_f (date/time applied already), but require valid hour for visuals
...
st.caption("v3.7: Agent Set radio (All / B2C / MT Team) + full **date+time** window filter (IST). Rows without Time can be included within the date window. 'MT Team' corresponds to 'MD'.")
